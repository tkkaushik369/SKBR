...
export class Utility {
	...
	static createCapsuleGeometry(radius = 1, height = 2, N = 32) {
		...
		return new THREE.CapsuleGeometry(radius, height, N / 2, N) //geometry;
	}
	...
	static appplyVectorMatrixXZ(a: THREE.Vector3, b: THREE.Vector3): THREE.Vector3 {
		const x: number = Number(Number(a.x * b.z) + Number(a.z * b.x)) / 2
		const z: number = Number(Number(a.z * b.z) + Number(-a.x * b.x)) / 2
		const vec = new THREE.Vector3(Number(x), 0, Number(z))
		// console.log(vec)
		return vec
	}
	...
	/* static createArray(length: number) {
	...
	static setDefaults(options: { [id: string]: any }, defaults: { [id: string]: any }) {
